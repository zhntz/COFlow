...
class TransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.self_attn = TransformerLayer(d_model=d_model, nhead=nhead, attention_type=attention_type, no_ffn=True,
                                          ffn_dim_expansion=ffn_dim_expansion, with_shift=with_shift)

        self.cross_attn_ffn = TransformerLayer(d_model=d_model, nhead=nhead, attention_type=attention_type,
                                               ffn_dim_expansion=ffn_dim_expansion, with_shift=with_shift)
...
class FeatureTransformer(nn.Module):
    ...
    def forward(self, feature0, feature1,
        ...
        assert self.d_model == c     #如果维度不相同，断言
        ...
        if self.attention_type == 'swin' and attn_num_splits > 1:
            ...
            window_size_w = w // attn_num_splits     ### // 4
...
class FeatureFlowAttention(nn.Module):
    ...
    def __init__(self, in_channels,
        ...
        self.q_proj = nn.Linear(in_channels, in_channels)    #输入输出形状相同
    ...
    def forward(self, feature0, flow,
        ...
        query = feature0.view(b, c, h * w).permute(0, 2, 1)  # [B, H*W, C]    调整维度+交换维度
