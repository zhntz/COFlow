...
class FLO_net(nn.Module):
    ...
    def forward(self,real_A,real_C,mb,attn_num_splits=64,dim=1,feature_channels=64):
        fake_occ1 = self.netG_occ(torch.cat((torch.cat([real_A, real_C], dim=1), mb), 1))       ### NOocc
        ...
        real_A,real_C = self.feature_add_position(real_A,real_C,attn_num_splits,feature_channels)        ### NOatt     Base+RES+OCC
        real_A,real_C = self.transformer(real_A,real_C,attn_num_splits)                       ### NOatt    Base+RES+OCC
        # feak_B = self.netA(torch.cat((torch.cat([real_A,real_C],dim=1),fake_occ), 1))      ### NOocc     Base+RES+CAFF
